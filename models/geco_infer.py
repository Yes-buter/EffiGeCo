...
class GeCo(nn.Module):
    ...
    def forward_cross(self, support_img, support_bboxes, query_img):
        ...
        support_img: (Batch, 3, H, W)
        support_bboxes: (Batch, N, 4)
        query_img: (Batch, 3, H, W)
        """
        # 1. Extract support features
        src_support, _ = self.backbone(support_img)
        
        # 2. Create prototypes from support image
        if not self.zero_shot:
        ...
        # 4. Adapt query features using support prototypes
        adapted_f = self.adapt_features(
        ...
        # 6. Refine boxes on query image
        masks, ious, corrected_bboxes = self.refine_bounding_boxes(src_query, outputs, return_masks=self.return_masks)

        for i in range(len(outputs)):
            outputs[i]["scores"] = ious[i]
            outputs[i]["pred_boxes"] = corrected_bboxes[i].to(outputs[i]["pred_boxes"].device).unsqueeze(0) / query_img.shape[-1]
